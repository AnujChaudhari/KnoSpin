...
export default function LibraryPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Safe concurrent initialization pipeline
    ...
    <div className="min-h-screen bg-gray-50/50 dark:bg-transparent pb-16">
      ...
      <div className="sticky top-0 z-40 bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-gray-100 dark:border-white/5 pt-6 pb-4 px-4 sm:px-6">
        <div className="max-w-3xl mx-auto">
          ...
          {/* ===== NATIVE APP-STYLE HORIZONTAL PILL CATEGORIES ===== */}
      ...
      <div className="max-w-3xl mx-auto px-4 mt-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {filteredBooks.map(book => (
            <div 
              key={book.id} 
              className="group bg-white dark:bg-[#111] border border-gray-100 dark:border-white/5 rounded-3xl p-4 shadow-sm hover:shadow-md transition-all duration-300"
            >
              {/* Premium Book Showcase Image */}
              <div className="relative w-full h-44 rounded-2xl overflow-hidden mb-4 bg-gray-100 dark:bg-white/5 border border-gray-50 dark:border-transparent">
                <img
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-2 mt-1.5 px-0.5 leading-relaxed">
                {book.description || "No description provided for this resource library document."}
              </p>

              {/* Compact One-Line Micro Actions */}
              <div className="flex items-center gap-2 mt-4 w-full overflow-hidden">
                {book.previewUrl && (
                  <button
                    onClick={() => setPreviewUrl(book.previewUrl)}
                    className="flex-1 flex justify-center items-center gap-1.5 px-3 py-2.5 bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 rounded-xl text-gray-800 dark:text-gray-200 text-xs font-bold transition-all active:scale-95 truncate"
                  >
                    <EyeIcon /> Preview
                  </button>
                )}
                {book.downloadUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex-1 flex justify-center items-center gap-1.5 px-3 py-2.5 bg-primary-600 hover:bg-primary-700 text-white text-xs font-bold rounded-xl transition-all active:scale-95 text-center shadow-sm shadow-primary-500/10 truncate"
                  >
